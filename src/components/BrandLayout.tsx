...
import {
  LayoutDashboard, Search, Heart, Megaphone, MessageSquare,
  BarChart3, LogOut, Menu, X, ChevronRight
} from "lucide-react";
import { getBrandSession, brandLogout } from "@/lib/adminStore";
import { Button } from "@/components/ui/button";
import ThemeToggle from "@/components/ThemeToggle";

const menuItems = [
  { icon: LayoutDashboard, label: "Dashboard", path: "/brand/dashboard" },
  { icon: Search, label: "Discover Influencers", path: "/brand/discover" },
  { icon: Heart, label: "Saved Influencers", path: "/brand/saved" },
    ...
    <div className="min-h-screen flex bg-background">
      <aside className={`hidden lg:flex flex-col ${sidebarOpen ? "w-64" : "w-20"} transition-all duration-300 bg-sidebar border-r border-sidebar-border`}>
        <div className="flex items-center gap-3 h-16 px-4 border-b border-sidebar-border">
          ...
          {sidebarOpen && <span className="font-display font-bold text-lg text-sidebar-foreground truncate">{brand.brandName}</span>}
        </div>
        ...
        <header className="h-16 flex items-center justify-between px-4 lg:px-6 border-b border-border bg-card/80 backdrop-blur-lg sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-display font-semibold text-lg text-foreground">
              {menuItems.find(i => i.path === location.pathname)?.label || "Brand Dashboard"}
            </h2>
          </div>
            ...
            <motion.aside initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} transition={{ type: "spring", damping: 25, stiffness: 200 }}
              ...
              <div className="flex items-center justify-between h-16 px-4 border-b border-sidebar-border">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-display font-bold text-sidebar-foreground">{brand.brandName}</span>
                </div>
