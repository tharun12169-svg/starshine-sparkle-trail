...
import {
  LayoutDashboard, MessageSquare, UserCheck, Users, UserPlus,
...
const menuItems = [
  ...
  { icon: Megaphone, label: "Campaign Requests", path: "/admin/campaigns" },
    ...
    <div className="min-h-screen flex bg-background">
      ...
      <aside className={`hidden lg:flex flex-col ${sidebarOpen ? "w-64" : "w-20"} transition-all duration-300 bg-sidebar border-r border-sidebar-border`}>
        <div className="flex items-center gap-3 h-16 px-4 border-b border-sidebar-border">
          ...
          {sidebarOpen && <span className="font-display font-bold text-lg text-sidebar-foreground">InfluenceHub</span>}
        </div>
        ...
        {mobileOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-foreground/20 z-40 lg:hidden" onClick={() => setMobileOpen(false)} />
            <motion.aside
            ...
            >
              <div className="flex items-center justify-between h-16 px-4 border-b border-sidebar-border">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-display font-bold text-sidebar-foreground">InfluenceHub</span>
                </div>
                ...
                {menuItems.map(item => {
                  ...
                  return (
                    <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)}
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${
                        active ? "gradient-bg text-primary-foreground" : "text-muted-foreground hover:text-sidebar-foreground hover:bg-sidebar-accent"
                      }`}>
              ...
              <div className="p-3 border-t border-sidebar-border">
                <button onClick={handleLogout}
                  className="flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium text-muted-foreground hover:text-destructive">
